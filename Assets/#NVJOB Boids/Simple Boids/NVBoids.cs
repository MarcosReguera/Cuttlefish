...
public class NVBoids : MonoBehaviour
{
    ...
    void FlocksMove()
    {
        ...
        for (int f = 0; f < flockNum; f++)
        {
            ...
            flocksTransform[f].localPosition = Vector3.SmoothDamp(flocksTransform[f].localPosition, FolksChase.transform.localPosition - flocksTransform[f].localPosition, ref velFlocks[f], 2);
        }
    ...
    void CreateBird()
    {
        ...
        for (int b = 0; b < birdsNum; b++)
        {
            ...
            birdsTransform[b].localRotation = Quaternion.Euler(0, Random.Range(-2, 2), 0);
            curentFlock[b] = Random.Range(0, flockNum);
            birdsSpeed[b] = Random.Range(4.5f, 6.0f);
        }
